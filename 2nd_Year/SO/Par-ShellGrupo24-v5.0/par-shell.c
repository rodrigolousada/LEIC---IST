...
int status, iteration = -1, counter = 0, exiter = 0, total_time = 0, nterminals = 0, terminalpids[MAX];
list_t* list;
FILE *myfile;
...
int main() {
	...
	while(1) {
		...
		if(strcmp(argVector[0],"exit-global")==0) /* For exit */ {
			pthread_mutex_lock(&Locker);
			exiter = 1;
		...
		if(strcmp(argVector[0], "pidterminal")==0) {
